...
- **A contract cannot be paid, and could not do anything with it if it were.** Nothing sends
  ether — there is no `CALL`, no `DELEGATECALL` and no `SELFDESTRUCT` in the opcode table — so
  anything that arrived would stay. Reading what a transaction carried was written and taken
  back out for that reason: a program that can count what it was paid and never move it is a
  keyword spent on a one-way box. Both come back together, and what they wait for is an
  external call — the first thing that would leave the program while it runs, and what the
  `Escapes` effect was named for.
